...
public class StoreDAO implements StoreDaoInterface {
	...
	public void addProductsToStore(int supplierId, int quantity) throws Exception {
		...
		PreparedStatement p = con.prepareStatement("select quantity from supplierproducts where id ="+supplierId);
		...
		if(result.getInt("quantity")-quantity < 0) {
			...
			PreparedStatement pstmt = con.prepareStatement("select productsinstore.productid from productsinstore,supplierproducts where productsinstore.productid =supplierproducts.productid and supplierproducts.id="+supplierId);
			
			if(pstmt.executeUpdate() == 0) {
				PreparedStatement pstmt1 = con.prepareStatement("select * from supplierproducts where id ="+supplierId+" and quantity >="+quantity+"");
				...
				if(pstmt2.executeUpdate() != 0) {
					PreparedStatement pstmt3 = con.prepareStatement("update supplierproducts set quantity = quantity-? where id = ?");
					pstmt3.setInt(1,quantity);
					pstmt3.setInt(2,supplierId);
					pstmt3.executeUpdate();
				}
			}else {
				PreparedStatement ps = con.prepareStatement("select * from supplierproducts where id ="+supplierId+" and quantity >="+quantity+"");
